...
function logNotification({ meetingId, memberId = null, type, channel = 'email', subject, content, recipient = null }) {
  const stmt = db.prepare(`
    INSERT INTO notifications (meeting_id, member_id, type, channel, subject, content)
    VALUES (?, ?, ?, ?, ?, ?)
  `);
  return stmt.run(meetingId, memberId, type, channel, subject, content).lastInsertRowid;
}
...
function sendAvailabilityRequest(meeting, member, token, baseUrl) {
  ...
  for (const { channel, recipient } of channels) {
    ...
    ids.push(logNotification({ meetingId: meeting.id, memberId: member.id, type: 'availability_request', channel, subject, content }));
  }
...
function sendReminder(meeting, member, token, baseUrl) {
  const link = `${baseUrl}/avail/${token}`;
  const channels = pickChannels(meeting, member);
  const ids = [];
  for (const { channel } of channels) {
    ...
    ids.push(logNotification({ meetingId: meeting.id, memberId: member.id, type: 'reminder', channel, subject, content }));
  }
  if (channels.some(c => c.channel === 'sms')) {
    db.prepare(`UPDATE member_tokens SET last_sms_at = CURRENT_TIMESTAMP WHERE meeting_id = ? AND member_id = ? AND purpose = 'availability'`)
      .run(meeting.id, member.id);
...
function sendMeetingConfirmed(meeting, member, slot) {
  const channels = pickChannels(meeting, member);
  const ids = [];
  for (const { channel } of channels) {
    let subject, content;
    ...
    ids.push(logNotification({ meetingId: meeting.id, memberId: member.id, type: 'meeting_confirmed', channel, subject, content }));
  }
  return ids;
}

function sendSignatureRequest(meeting, member, token, baseUrl) {
  ...
  for (const { channel } of channels) {
    ...
    ids.push(logNotification({ meetingId: meeting.id, memberId: member.id, type: 'signature_request', channel, subject, content }));
  }
  return ids;
}
